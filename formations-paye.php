...
$resultats = $connexion->query( "SELECT * from formation where id_formation='".$_GET["id"]."'" );
...
if ($_SESSION["admin"]  == true){
  ...
  $nom=$resultat->nom;
	...
	header("Location: formations-paye-admin.php?nom=$nom ");
}}
// if (($_SESSION["paye"]==0)&&($_SESSION["admin"]  != true)){
 // $nom=$resultat->nom;
 // header("Location: formations.php?nom=$nom");
//}

?>
<?php  $nom=$_GET["nom"];
$categ=$_SESSION['paye'];
if ($_GET['id']!=$_SESSION['paye']){
  $id=$_GET['id'];
  header("Location: formations.php?id=$id");
}
/*$resultats = $connexion->query("SELECT * from formation-paye where type='".$nom."' and categorie='".$categ."'");
$resultats->setFetchMode(PDO::FETCH_OBJ);
$resultats = $connexion->query( "SELECT * from formation where id_formation='".$_GET["id"]."'" );
$resultats->setFetchMode( PDO::FETCH_OBJ );
$resultat = $resultats->fetch() ;
        if ($resultats->rowCount() <= 0) {
      header("Location: stripe/form-paiement.php?total=5&&nom=$resultat->nom&&id=$resultat->id_formation");
    } */
...
<!--*********************************************************************************************************-->
 ...
 <div class="formations">
       <?php 
        ...
        $resultats_1 = $connexion->query( "SELECT * from formation_paye where categorie='".$_GET["id"]."'  " );
